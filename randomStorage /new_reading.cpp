...
vector<location> intersection(vector<location> c1 , vector<location> c2){
    ...
    // cout << "in here in intersection" << endl ; 
...
vector< vector<location> > merging_clusters( vector< vector<location> > current) {
    ...
    while(!has_array){
        ...
        if(merged.empty()) { 
            ...
            if( test_1.size() > merged[0].size() / 5 ) {
                ...
                //creating a set here to test it out instead 
                set<location> new_cur ; 
                int size = merged[0].size() ; 
                for (int k = 0 ; k < size; k++){
                    new_cur.insert( merged[0][k] ) ; 
                }
                merged[0].assign( new_cur.begin(), new_cur.end() ) ; 
                sort(merged[0].begin(), merged[0].end(), sortByLocation) ; 
            }else{ 
    ...
    //version four on this new placement of the second half of this goddam loop
    cout << "in the second loop here " << endl ; 
    //apparently, after the 1st position of location_counter, there's a seg fault at the first intersection of that location'
    location_counter = 1 ; //this is a bookmark for merged. 

    for (int i = 0 ; i < locations.size() ; i++){
        //this here is to set a new cluster assuming locations[i]th cluster does match any of them 
        if( find(finished.begin(), finished.end(), locations[i]) != finished.end()){
            //making sure I haven't used locations[i] and moving on here '
            continue ; 
        }
        merged.push_back(current[locations[i]]) ;
        finished.push_back(locations[i]) ;  

        for(int j = i + 1 ; j < locations.size() ; j++){
            // at i + 1 because there's no point in going over the same cluster twice '
            if( find(finished.begin(), finished.end(), locations[j]) != finished.end()) {
                continue ; 
            }
            vector<location> test_1, test_2 ; 
            test_1 = intersection(merged[location_counter], current[locations[j]]) ; 
            if( test_1.size() > merged[location_counter].size() / 5 ){
                test_2 = unique_difference(merged[location_counter], current[locations[j]]) ; 
                merged[location_counter].insert( merged[location_counter].end(), test_2.begin(), test_2.end() ) ; 
                cout << "four" << endl ; 
                finished.push_back(locations[j]) ; 
            }
        }
    }
    return merged ; 
...
bool sortByLocation(const location &lhs , const location &rhs){
    if(lhs.position1 < rhs.position1){
        return lhs.position1 < rhs.position2 ; 
    }else{
