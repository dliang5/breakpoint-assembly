...
for j in range(2):
    ...
    if j == 0:
        ...
        targetCount = 6 # common inversion count score
    # remove everything up from this area to keep it back the original code style.
    # starting permutation iteration here
    for i in range(1000):
        hit = 0
        count = 0
        already_list = set() # this is used to test for both first and second positions of the gene
        ...
        for index, invEntry in enumerate(inversionList[j]):
            ...
            high1 = pos(low1.getpos() + (invEntry.getlpos2() - invEntry.gethpos2()))
            high2 = pos(low1.getpos() + invEntry.gethpos2() )
            #print("{} {} {} {}".format(str(low1.getpos()), str(low2.getpos()), str(high1.getpos()), str(high2.getpos())))
            ...
            for gene in geneList:
                ...
                if (invEntry.getChromRef(), low1.getgene()) in already_list or (invEntry.getChromRef(), low2.getgene()) in already_list or (invEntry.getChromRef(), high1.getgene()) in already_list or (invEntry.getChromRef(), high2.getgene()) in already_list:
                    continue
                # comparing to see if they have matching gene positions or not
                if(low1.getgene() == low2.getgene()):
                    already_list.add((invEntry.getChromRef(), low1.getgene()))
                    hit += 1
                    check_counter += 1
                    lflag = 1
                if (high1.getgene() == high2.getgene()):
                    already_list.add((invEntry.getChromRef(), high1.getgene()))
                    hit += 1
                    check_counter += 1
                    hflag = 1
                if check_counter > 0:
