...
for j in range(2):
    targetCount = 0
    if j == 0:
        targetCount = 26 # rare inversion count score
    elif j == 1:
    ...
    for i in range(1000):
        ...
        for invEntry in inversionList[j]:
            ...
            ### might have to delete this as it's guarantee that it will always get a mark.
            for gene in geneList:
                ...
                if(low1.getgene() == low2.getgene() and low1.getgene() not in already_list):
                    already_list.add(low1.getgene())
                    check_counter += 1
                    hit += 1
                if (high1.getgene() == high2.getgene() and high1.getgene() not in already_list):
                ...
                if check_counter > 0:
                    ...
                    break
        print("{} {}".format(j, hit))
        # if the hit count is above observation then increment
        if hit >= targetCount:
            meanList[j] += 1
# getting the results here
print("Producing the over values")
